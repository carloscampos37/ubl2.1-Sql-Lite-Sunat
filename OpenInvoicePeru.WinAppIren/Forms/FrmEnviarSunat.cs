    ...
    public partial class FrmEnviarSunat : Form
    {
        ...
        private DataTable DetalleArticulos(DataTable dt, int iii)
        {
            try
            {
                ...
                SQL = " SELECT ProductodescripcionP=isnull(d.ProductoDescripcion,''''),Producto,UnidadMedidaId,b.*, c.*,e.* FROM ";
        ...
        public static DetalleDocumento DetalleItem(DataTable DtArticulos, int i)
        {
            ...
            try
            {
                ...
                string vId = Convert.ToString(DtArticulos.Rows[i]["Id"]);
                string vAfectacionID = Convert.ToString(DtArticulos.Rows[i]["Id2"]);

                decimal vImponible = Convert.ToDecimal(DtArticulos.Rows[i]["ItemGravado"]);
                if(vAfectacionID == "E")
                {
                    detalle.ItemGravado =  Convert.ToDecimal(DtArticulos.Rows[i]["ItemExonerado"]);

                }
                if (vAfectacionID == "I")
                {
                    detalle.ItemGravado =  Convert.ToDecimal(DtArticulos.Rows[i]["ItemInafecto"]);
     ...
     //       SQL = " SELECT a.empresaID, a.EstablecimientoID,Fechadocumento,b.*,Letras=[dbo].fn_NumeroEnLetra(TotalImporteVenta,'01'),AnexoSunat,d.* ";
            SQL = " SELECT a.empresaID, a.EstablecimientoID,Fechadocumento,b.*,Letras=[dbo].fn_NumeroEnLetra(TotalImporteVenta,'01'),AnexoSunat ";
            SQL += " FROM dbo.FEDocumentosElectronicosCab a ";
            SQL += "  INNER join FEDocumentosElectronicosDet b ON a.ID = b.DocumentosElectronicosCabID ";
            SQL += "  INNER join FeEstablecimientos c ON a.EstablecimientoID = c.ID ";
      //      SQL += "  INNER join FeAfectacionIgv d ON b.AfectacionIgv = d.ID ";
            SQL += " WHERE a.EmpresaID='" + Ctr_AyuEmpresas.Codigo + "' AND fechadocumento = '" + DtpFechaDoc.Text + "'";
            ...
            else
            {
                ...
                SQL += " AND a.EstadoAnulado=0 AND b.EstadoAnulado=0 Order by a.EstablecimientoID ";
            }
            ...
            try
            {
                if (DtDocumentos.Rows.Count > 0)
                {
                    ...
                    vArchivoXML = Ctr_AyuEmpresas.Codigo;
                    vArchivoXML = ModFunc.CreacionDirectorio(vRutaXml, vArchivoXML);

                    // Creacion de directorio de facturas electronicas

                    // Creacion de directorio año
                    vRutaXml = vArchivoXML;
                    vArchivoXML = Convert.ToString(DtpFechaDoc.Value.Year);
                    vArchivoXML = ModFunc.CreacionDirectorio(vRutaXml, vArchivoXML);

                    // Creacion de directorio mes

                    vRutaXml = vArchivoXML;
                    vArchivoXML = ModFunc.DesMes(ModFunc.Derecha("0" + Convert.ToString(DtpFechaDoc.Value.Month), 2));
                    vArchivoXML = ModFunc.CreacionDirectorio(vRutaXml, vArchivoXML);
