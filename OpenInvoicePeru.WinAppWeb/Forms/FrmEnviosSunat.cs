...
namespace OpenInvoicePeru.WinApp
{
    public partial class FrmEnviosSunat : Form
    {
            ...
            { 
                ...
                if (!rbResumen.Checked)
                {
                    ...
                    try
                    {
                        ...
                        {
                            vRutaArchivoCDR= $"{respuestaEnvio.NombreArchivo}";
                            vRutaArchivoCDR = Path.Combine(vRutaCdr, vRutaArchivoCDR);

                            File.WriteAllBytes(vRutaArchivoCDR, Convert.FromBase64String(respuestaFirmado.TramaXmlFirmado));

                            vRutaArchivoCDR = "R-" + $"{respuestaEnvio.NombreArchivo}";
                            vRutaArchivoCDR = Path.Combine(vRutaCdr, vRutaArchivoCDR);

                            File.WriteAllBytes(vRutaArchivoCDR, Convert.FromBase64String(rpta.TramaZipCdr));
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        private void BntGenera_Click(object sender, EventArgs e)
        {
            ...
            SQL = " SELECT a.EstablecimientoID,Fechadocumento,b.*,Letras=[dbo].fn_NumeroEnLetra(TotalImporteVenta,'01'),AnexoSunat,d.* ";
            SQL += " FROM FeEmpresas aa INNER JOIN dbo.FeDocumentosElectronicosCab a ON aa.ID=a.EmpresaID ";
            SQL = SQL + "  INNER join FeDocumentosElectronicosDet b ON a.ID = b.FEDocumentosElectronicosDet ";
             SQL = SQL + "  INNER join FeEstablecimientos c ON a.establecimientoID = c.ID ";
            SQL = SQL + " WHERE aa.ID='" + Ctr_AyuEmpresas.Codigo + "' AND fechadocumento = '" + DtpFechaDoc.Text + "'";
            ...
            try
            {
                if (DtDocumentos.Rows.Count > 0)
                {
                    ...
                    vRutaArchivoCDR = "CDR";
                    vRutaCdr = ModFunc.CreacionDirectorio(vRutaCdr, vRutaArchivoCDR);
                }
                switch (vTipoDoc)
                {
                    case "RA":
        ...
        private void Ctr_AyuUrlDoc_Load(object sender, EventArgs e)
        {
